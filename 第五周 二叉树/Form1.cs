...
namespace 第8周_二叉树
{
    ...
    public partial class MainForm : Form
    {
        ...
        // 添加按钮，在树的最左结点(即中序序列的第一个结点)插入一个孩子结点
        private void BTN_LEFT_INSERT_Click(object sender, EventArgs e)
        {
            string strt = "1234567890+-*/";
            foreach (char ch in TB_LEFT_INSERT.Text)
            {
                bool condition = ((ch >= 'A') && (ch <= 'Z')) || ((ch >= 'a') && (ch <= 'z')) || (strt.IndexOf(ch) >= 0);
                if (!condition)
                {
                    MessageBox.Show("请输入a-z或A-Z或0-9之间的字符。");
                    return;
                }
            }
            string initstring = tb_strin.Text;
            String INPUTSTRING = $"({TB_LEFT_INSERT.Text})";
            // 将需要插入的字符插入初始化的字符串，最后重新初始化
            for (int i = 0; i < initstring.Length; i++)
            {
                char ch = initstring[i];
                if (ch == ')' || ch == ','||ch==0)
                {
                    // 在此之前插入
                    initstring = initstring.Substring(0, i) + INPUTSTRING + initstring.Substring(i);
                    m_tree.CreateBinTreestr(initstring);
                    m_tree.DrawBTree(pb_bstree);
                    return;
                }
            }
            initstring += INPUTSTRING;
            m_tree.CreateBinTreestr(initstring);
            m_tree.DrawBTree(pb_bstree);
        }
        // 统计所有叶节点的个数
        ...
        {
            int  num = m_tree.GetLeavesNodeNum();
            MessageBox.Show("叶节点的个数为：" + num);
        }
        // 删除所有的叶节点
        private void button4_Click(object sender, EventArgs e)
        {
            m_tree.RemoveAllLeaveNodes();
            m_tree.DrawBTree(pb_bstree);
        }
